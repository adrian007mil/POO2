...
public class ConexionBD {
    ...
    private static final String DB_URL = "jdbc:sqlserver://localhost:1433;databaseName=BDVentas;trustServerCertificate=true;encrypt=false";
    ...
    public static List<Producto> obtenerProductosMayorCantidad() {
        List<Producto> productos = new ArrayList<>();
        String sql = "SELECT TOP 10 * FROM Producto WHERE Indicador = 'S' ORDER BY Stock DESC";
    ...
    public static List<Producto> obtenerProductosMayorPrecio() {
        List<Producto> productos = new ArrayList<>();
        String sql = "SELECT TOP 10 * FROM Producto WHERE Indicador = 'S' ORDER BY Precio DESC";
    ...
    public static String obtenerEstadisticasInventario() {
        ...
        try (Connection conn = getConexion()) {
            ...
            if (rs.next()) {
                stats.append("Valor total inventario: S/").append(String.format("%.2f", rs.getDouble("ValorTotal"))).append("\n");
            }
